#!/usr/bin/env bash
# Manage the EC2 instance defined by this Terraform configuration.
set -euo pipefail
...
case "$ACTION" in
  ...
  status)
    aws_ec2 describe-instances --instance-ids "$INSTANCE_ID" \
      --query 'Reservations[0].Instances[0].{InstanceId:InstanceId,State:State.Name,InstanceType:InstanceType,PublicIp:PublicIpAddress,PrivateIp:PrivateIpAddress}' \
      --output table
    ;;
esac
